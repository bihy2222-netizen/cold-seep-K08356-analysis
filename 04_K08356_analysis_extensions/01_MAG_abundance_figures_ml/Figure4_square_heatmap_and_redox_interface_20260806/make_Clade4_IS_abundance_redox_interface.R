...
depth_grid <- seq(0, 12, length.out = 250)
model_curves <- bind_rows(
  tibble(depth = depth_grid, relative = exp(-depth_grid / 3.0), component = "IO3− / NO3− availability"),
  tibble(depth = depth_grid, relative = plogis((depth_grid - 5.2) / 1.15), component = "Reduced sulfur availability"),
  tibble(depth = depth_grid, relative = exp(-0.5 * ((depth_grid - 6.0) / 1.8)^2), component = "Predicted Clade 4 niche")
) %>%
  mutate(component = factor(component, levels = c(
    "IO3− / NO3− availability", "Reduced sulfur availability", "Predicted Clade 4 niche"
  )))
model_colors <- c(
  "IO3− / NO3− availability" = "#3B6FB6",
  "Reduced sulfur availability" = "#C87B24",
  "Predicted Clade 4 niche" = "#7B4FA3"
)

p_redox <- ggplot(model_curves, aes(relative, depth, color = component, linetype = component)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 4.0, ymax = 8.0,
           fill = "#B39DDB", alpha = 0.16) +
  geom_path(aes(group = component), linewidth = 1.25) +
  scale_y_reverse(breaks = c(0, 4, 8, 12)) +
  scale_x_continuous(limits = c(0, 1.03), breaks = c(0, 0.5, 1)) +
  scale_color_manual(values = model_colors, name = NULL) +
  scale_linetype_manual(values = c("solid", "dashed", "solid"), name = NULL) +
  annotate("text", x = 0.51, y = 6.0, label = "Predicted redox-interface overlap",
           fontface = "bold", size = 3.0, color = "#4B3A60") +
  annotate("label", x = 0.77, y = 10.25,
           label = "Low-potential sulfur donors\n→ quinone/electron-transfer chain\n→ DIRM-like IdrABP complex\n→ high-potential oxyanion acceptor",
           size = 2.55, hjust = 0.5, fill = "white", linewidth = 0.25) +
  annotate("text", x = 0.02, y = 10.8,
           label = "Moco: DMSOR cofactor maturation\nTCA/rTCA: carbon flow and reducing power\nDsr/Sat: sulfur-redox background\nB12: cofactor autonomy",
           hjust = 0, size = 2.6, color = "#222222") +
  labs(
    x = "Relative availability / predicted niche", y = "Conceptual sediment depth",
    title = "D  Testable redox-interface model",
    subtitle = "Conceptual prediction only—not a fitted geochemical profile or proof of iodate respiration"
  ) +
  theme_minimal(base_size = 8.5) +
  theme(panel.grid.minor = element_blank(), plot.title = element_text(face = "bold", size = 10.2),
        plot.subtitle = element_text(size = 7.6), legend.position = "bottom")

figure <- p_abundance_matrix / (p_station_depth | p_depth | p_redox) +
  plot_layout(heights = c(1.25, 1.0), widths = c(0.86, 1.0, 1.1)) +
  plot_annotation(
    title = "Clade 4 carrier abundance varies among IS stations and motivates a redox-interface hypothesis",
    subtitle = "Observed MAG TPM patterns are separated from the conceptual prediction of overlap between high-potential oxyanions and reduced sulfur",
    caption = paste0(
      ...
      "Thus, the interface peak in panel D is a falsifiable ecological prediction rather than a pattern demonstrated by these three depth intervals. ",
      "IO3−/I−, Eh, O2, nitrate and reduced-sulfur measurements are required for direct testing."
    ),
    theme = theme(
...
save_plot <- function(extension, device, dpi = 450) {
  args <- list(
    filename = file.path(out_dir, paste0("Clade4_IS_abundance_redox_interface.", extension)),
...
qc <- c(
  "Clade 4 IS abundance and redox-interface figure QC",
  "Status: PASSED",
  ...
  "Conceptual interface peak explicitly labeled as a testable prediction, not observed evidence.",
  "No iodate-respiration or IdrA activity claim is made."
)
writeLines(qc, file.path(out_dir, "Clade4_IS_abundance_redox_interface_QC.txt"))
