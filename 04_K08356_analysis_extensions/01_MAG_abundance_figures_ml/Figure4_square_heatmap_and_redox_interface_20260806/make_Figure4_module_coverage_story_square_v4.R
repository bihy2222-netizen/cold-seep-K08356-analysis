...
ranked <- read.csv(file.path(source_dir, "predefined_module_statistics_ranked.csv"),
                   check.names = FALSE, stringsAsFactors = FALSE)
...
write.csv(weighted_potential,
          file.path(out_dir, "Figure4_square_v4_abundance_weighted_potential_56samples.csv"),
          row.names = FALSE)
...
middle_row <- wrap_plots(p_distribution, p_forest, p_stability, nrow = 1)
figure4_v3 <- wrap_plots(panel_a, middle_row, p_weighted, ncol = 1,
                         heights = c(0.78, 1.05, 1.10)) +
  plot_annotation(
...
save_figure <- function(extension, device, dpi = 450) {
  args <- list(filename = file.path(out_dir, paste0("Figure4_module_coverage_story_square_v4.", extension)),
               plot = figure4_v3, width = 19, height = 14.2, units = "in", bg = "white",
               device = device)
...
writeLines(c(figure_qc, "Heatmap tile aspect ratio: 1:1 (square)."),
           file.path(out_dir, "Figure4_module_coverage_story_square_v4_QC.txt"))
