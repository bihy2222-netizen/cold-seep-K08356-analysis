...
outdir <- '/Users/catherine/Downloads/aioA 蛋白序列建树/TPM_combo_figure_20260728/overall_derep_48seq47MAG_20260802'
...
pA <- ggplot(contig_k08356, aes(habitat, logTPM, fill=habitat)) +
  ...
  labs(title='A  Overall-derep contig-level K08356 abundance', x=NULL, y='log10(TPM + 1)') +
  base_theme +
...
sample_box_long <- mag_host_long %>%
  filter(MAG_subclass != 'mixed K08356 subclasses') %>%
  mutate(MAG_subclass=factor(MAG_subclass, levels=class_levels[1:4]),
         sample=factor(sample, levels=sample_order))

pB <- ggplot(sample_box_long, aes(sample, logTPM, fill=habitat)) +
  geom_boxplot(width=0.62, outlier.shape=NA, alpha=0.72, color='grey25', linewidth=0.18) +
  geom_jitter(aes(color=habitat), width=0.11, height=0, size=0.55, alpha=0.52, show.legend=FALSE) +
  ...
  labs(title='D  Overall-derep host-MAG abundance across 56 samples',
       x=NULL, y='Host MAG log10(TPM + 1)') + base_theme +
  theme(axis.text.x=element_text(angle=65, hjust=1, size=3.8),
...
pD <- ggplot(heat_df, aes(sample, MAG_ID, fill=logTPM)) +
  ...
  scale_fill_gradientn(colors=c('#2166ac','#f7f7f7','#f4a582','#b2182b'), name='log10(TPM+1)') +
  labs(title='C  Overall derep: 47 unique K08356-bearing host MAGs', x=NULL, y=NULL) + base_theme +
...
title <- ggdraw() +
  draw_label('Overall-dereplicated K08356 abundance: 48 sequences, 47 host MAGs, 56 samples', x=0, y=0.72, hjust=0, fontfamily='Times', fontface='bold', size=15) +
  draw_label('Overall-derep version (clade counts 3/6/19/20). S1_9-12_bin1 is retained once in the MAG heatmap and excluded from mutually exclusive subclass summaries.', x=0, y=0.24, hjust=0, fontfamily='Times', size=9)
fig <- plot_grid(title, body, ncol=1, rel_heights=c(0.09,1))

pdf_file <- file.path(outdir, 'overall_derep_K08356_48seq47MAG_56sample.pdf')
png_file <- file.path(outdir, 'overall_derep_K08356_48seq47MAG_56sample.png')
svg_file <- file.path(outdir, 'overall_derep_K08356_48seq47MAG_56sample.svg')
ggsave(pdf_file, fig, width=15.2, height=12.2, units='in', device=cairo_pdf, bg='white')
ggsave(png_file, fig, width=15.2, height=12.2, units='in', dpi=300, bg='white')
ggsave(svg_file, fig, width=15.2, height=12.2, units='in', device=svg, bg='white')
...
write.csv(sample_box_long, file.path(outdir,'panelD_56sample_host_MAG_boxplot_long.csv'), row.names=FALSE)
...
qc <- c(
  paste('dataset_version', 'overall_derep_48seq47MAG', sep='\t'),
  ...
  paste('sequence_clade_counts', '3/6/19/20', sep='\t'),
