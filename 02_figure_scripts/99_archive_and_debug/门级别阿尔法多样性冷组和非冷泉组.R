...
# 创建新的分组数据框 - 只有cold_seep和non-seep两组
group_data <- data.frame(
  sample.id = c(
    # cold_seep组 (48个样品)
    "SY365BB-0-4", "SY365BB-4-8", "SY365BB-8-12", "SY366YB-0-4", "SY366YB-4-8", 
    "SY366YB-8-12", "SY366YW-0-4", "SY366YW-4-8", "SY366YW-8-12", "SY368YW-0-4", 
    "SY368YW-4-8", "SY368YW-8-12", "SY456YB-0-4", "SY456YB-4-8", "SY456YB-8-12", 
    "SY457BB-0-4", "SY457BB-4-8", "SY457BB-8-12", "SY459WG-0-4", "SY459WG-4-8", 
    "SY459WG-8-12", "S4_12-15", "S4_9-12", "SQ_58_0-4", "SQ_58_4-8", "SQ_58_8-12", 
    "SQ_81_0-4", "SQ_81_4-8", "SQ_81_8-12", "S2_0-3", "S2_3-6", "S2_12-15", 
    "S1_0-3", "S1_6-9", "S1_9-12", "C2_0-6", "C2_12-18", "C2_6-12", "C1_0-6", 
    "C1_12-18", "C1_6-12", "C3_0-6", "C3_12-18", "C3_6-12", "S13_0-2", 
    "S14_4-6", "S15_8-10", "ES_2_0-6", 
    # non_seep组 (8个样品)
  ...
  group = c(
    rep("cold_seep", 48),  # 前48个样品是cold_seep
    rep("non_seep", 8)     # 后8个样品是non-seep
  )
)
...
cat("分组统计: cold_seep = 48个样品, non_seep = 8个样品\n")
...
create_mock_phylum_data <- function(sample_names) {
  ...
  for(i in 1:n_samples) {
    # 根据组别调整数据分布
    if (group_list[i] == "cold_seep") {
      # cold_seep组: 高多样性
      abundances <- runif(n_phyla, 10, 30)
    } else if (group_list[i] == "non_seep") {
      # non_seep组: 较低多样性
...
group_col <- c(cold_seep = "#00FF7F", non_seep = "#FF1493")
...
create_boxplot_with_detailed_results <- function(data, value_col, label, color_palette, detailed_results) {
  ...
  step_height <- (y_limits[2] - max_value) / 8  # 为比较预留空间
  ...
  # 添加Wilcoxon检验结果（由于只有两组，不再使用Kruskal-Wallis）
  if (nrow(significant_comparisons) > 0) {
    comp <- significant_comparisons[1, ]
    wilcoxon_label <- paste0("Wilcoxon test: p = ", format(comp$p_value_adjusted, digits = 3))
  } else {
    # 如果没有显著差异，显示p值
    comp <- detailed_results[1, ]
    wilcoxon_label <- paste0("Wilcoxon test: p = ", format(comp$p_value_adjusted, digits = 3))
  }
  
  p <- p + annotate("text", x = 1.5, y = y_limits[2] * 0.98, 
                    label = wilcoxon_label, size = 3.5, color = "black", hjust = 0.5)
  
  # ===== 修改点2：添加p值校正说明 =====
  p <- p + annotate("text", x = 1.5, y = y_limits[2] * 0.93, 
...
# 由于只有两组，直接使用Wilcoxon检验结果
wilcoxon_results <- data.frame()
for (idx in c("Shannon", "Simpson", "Observed_Species", "Chao1")) {
  test_result <- wilcox.test(alpha_df[[idx]] ~ alpha_df$Group, exact = FALSE, conf.int = TRUE)
  results_df <- if (idx == "Shannon") shannon_detailed else 
    if (idx == "Simpson") simpson_detailed else
      if (idx == "Observed_Species") observed_detailed else chao1_detailed
  
  wilcoxon_results <- rbind(wilcoxon_results, data.frame(
    Index = idx,
    W_statistic = round(test_result$statistic, 3),
    p_value_raw = round(test_result$p.value, 6),
    p_value_adjusted = results_df$p_value_adjusted,
    Significance = results_df$Significance,
    Effect_size_r = results_df$Effect_size_r,
    stringsAsFactors = FALSE
  ))
}
...
write.csv(wilcoxon_results, "wilcoxon_test_results_phylum_corrected.csv", row.names = FALSE)
...
cat("原因: 虽然只有1组比较，但为保持一致性仍进行校正\n")
cat("过程: 对每个多样性指数进行比较和校正\n")
...
cat("12. wilcoxon_test_results_phylum_corrected.csv - Wilcoxon检验结果\n")
cat("13. descriptive_statistics_phylum_corrected.csv - 描述性统计\n")
cat("14. p_value_correction_notes.csv - p值校正方法说明\n")

cat("\n统计检验说明:\n")
cat("- 只有两组比较，直接使用Wilcoxon秩和检验\n")
cat("- 应用Benjamini-Hochberg FDR校正保持方法一致性\n")
