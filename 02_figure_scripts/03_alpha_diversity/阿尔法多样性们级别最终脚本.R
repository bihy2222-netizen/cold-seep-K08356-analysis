...
# 函数：执行Wilcoxon检验并返回详细结果
perform_detailed_wilcox_test <- function(data, value_col) {
  results <- data.frame()
  ...
  for (pair in comparison_pairs) {
    ...
    significance <- ifelse(p_val < 0.0001, "****",
                           ifelse(p_val < 0.001, "***",
                                  ifelse(p_val < 0.01, "**",
...
# 为每个指数执行详细检验
cat("\n1. Shannon指数详细检验结果:\n")
shannon_detailed <- perform_detailed_wilcox_test(alpha_df, "Shannon")
print(shannon_detailed)

cat("\n2. Simpson指数详细检验结果:\n")
simpson_detailed <- perform_detailed_wilcox_test(alpha_df, "Simpson")
print(simpson_detailed)

cat("\n3. Observed Species详细检验结果:\n")
observed_detailed <- perform_detailed_wilcox_test(alpha_df, "Observed_Species")
print(observed_detailed)

cat("\n4. Chao1指数详细检验结果:\n")
chao1_detailed <- perform_detailed_wilcox_test(alpha_df, "Chao1")
print(chao1_detailed)

# 保存详细检验结果
write.csv(shannon_detailed, "shannon_detailed_pairwise_results_phylum.csv", row.names = FALSE)
write.csv(simpson_detailed, "simpson_detailed_pairwise_results_phylum.csv", row.names = FALSE)
write.csv(observed_detailed, "observed_species_detailed_pairwise_results_phylum.csv", row.names = FALSE)
write.csv(chao1_detailed, "chao1_detailed_pairwise_results_phylum.csv", row.names = FALSE)
cat("\n详细成对比较结果已保存\n")
...
# 保存PDF矢量图 - 组合图
ggsave("alpha_diversity_boxplot_phylum.pdf", plot = combined_plot, 
       width = 14, height = 12)
cat("组合箱线图已保存为PDF矢量图: alpha_diversity_boxplot_phylum.pdf\n")

# ===== 保存各个单独指数的PDF - 修复这里 =====
cat("\n=== 保存单独指数的PDF矢量图 ===\n")

# 1. Shannon指数单独图
ggsave("shannon_diversity_boxplot_phylum.pdf", plot = shannon_plot, 
       width = 8, height = 8)
cat("Shannon指数单独图已保存: shannon_diversity_boxplot_phylum.pdf\n")

# 2. Simpson指数单独图
ggsave("simpson_diversity_boxplot_phylum.pdf", plot = simpson_plot, 
       width = 8, height = 8)
cat("Simpson指数单独图已保存: simpson_diversity_boxplot_phylum.pdf\n")

# 3. Observed Species单独图
ggsave("observed_species_boxplot_phylum.pdf", plot = observed_plot, 
       width = 8, height = 8)
cat("Observed Species单独图已保存: observed_species_boxplot_phylum.pdf\n")

# 4. Chao1指数单独图
ggsave("chao1_index_boxplot_phylum.pdf", plot = chao1_plot, 
       width = 8, height = 8)
cat("Chao1指数单独图已保存: chao1_index_boxplot_phylum.pdf\n")
...
if (nrow(all_significant) > 0) {
  ...
  sig_summary_plot <- ggplot(all_significant, aes(x = Comparison, y = Index, fill = Significance)) +
    ...
    labs(title = "Alpha Diversity: Significant Pairwise Comparisons",
  ...
  ggsave("significance_summary_phylum.pdf", plot = sig_summary_plot, 
         width = 10, height = 6)
  cat("显著性差异汇总图已保存: significance_summary_phylum.pdf\n")
} else {
  cat("警告：没有发现显著差异！\n")
}
...
write.csv(kruskal_results, "kruskal_wallis_results_phylum.csv", row.names = FALSE)
...
write.csv(desc_stats, "descriptive_statistics_phylum.csv", row.names = FALSE)

# ===== 最终输出总结 =====
cat("\n")
cat(paste(rep("=", 60), collapse = ""))
cat("\n")
cat("ALPHA多样性分析完成总结（门水平）\n")
cat(paste(rep("=", 60), collapse = ""))
cat("\n\n")
...
cat("\n生成的所有文件:\n")
cat("1. alpha_diversity_boxplot_phylum.pdf - 组合箱线图（A,B,C,D）\n")
cat("2. shannon_diversity_boxplot_phylum.pdf - Shannon指数单独箱线图\n")
cat("3. simpson_diversity_boxplot_phylum.pdf - Simpson指数单独箱线图\n")
cat("4. observed_species_boxplot_phylum.pdf - Observed Species单独箱线图\n")
cat("5. chao1_index_boxplot_phylum.pdf - Chao1指数单独箱线图\n")
cat("6. significance_summary_phylum.pdf - 显著性差异汇总图\n")
cat("7. alpha_diversity_results_phylum.csv - 原始多样性数据\n")
cat("8. shannon_detailed_pairwise_results_phylum.csv - Shannon成对比较\n")
cat("9. simpson_detailed_pairwise_results_phylum.csv - Simpson成对比较\n")
cat("10. observed_species_detailed_pairwise_results_phylum.csv - Observed Species成对比较\n")
cat("11. chao1_detailed_pairwise_results_phylum.csv - Chao1成对比较\n")
cat("12. kruskal_wallis_results_phylum.csv - Kruskal-Wallis检验结果\n")
cat("13. descriptive_statistics_phylum.csv - 描述性统计\n")

cat("\n统计检验说明:\n")
cat("- 所有成对比较使用Wilcoxon秩和检验\n")
cat("- 显著性水平: **** p<0.0001, *** p<0.001, ** p<0.01, * p<0.05\n")
cat("- 整体差异检验使用Kruskal-Wallis检验\n")

cat("\n图形说明:\n")
cat("- 所有图形均为PDF矢量图格式，可无限放大不失真\n")
cat("- 每个箱线图包含：箱体（25-75%四分位距）、中位数线、散点（原始数据）\n")
cat("- 显著性标注显示在图形上方\n")
cat("- 组合图包含4个子图：Shannon, Simpson, Observed Species, Chao1\n")
