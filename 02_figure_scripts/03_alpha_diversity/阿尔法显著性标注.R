...
# ===== 新增：使用ggbarplot绘制Shannon指数的条形图并进行统计检验 =====
...
shannon_plot <- ggbarplot(alpha_df, x = "Group", y = "Shannon", fill = "Group",
                          ...
                          add = "mean_se", label = FALSE) +
  labs(title = "Shannon Diversity Index by Group",
       x = "Group", y = "Shannon Diversity Index") +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))

# 添加ANOVA检验结果
shannon_plot <- shannon_plot + 
  stat_compare_means(method = "anova", label.y = max(alpha_df$Shannon) * 1.1, 
                     label.x = 0.8, label = 'p.format')

# 添加成对比较（Wilcoxon检验）
shannon_plot <- shannon_plot +
  stat_compare_means(comparisons = list(c('IS','AS'),
                                        c('IS','ES'),
                                        c('IS','NS'),
                                        c('AS','ES'),
                                        c('AS','NS'),
...
# 保存Shannon指数条形图
ggsave("shannon_diversity_barplot.png", plot = shannon_plot, width = 8, height = 6, dpi = 300)
cat("Shannon指数条形图已保存: shannon_diversity_barplot.png\n")
...
cat("- Shannon指数条形图: shannon_diversity_barplot.png\n")
if (any(alpha_test_results$p_value < 0.05)) {
  cat("- 事后检验结果: posthoc_dunn_*.csv\n")
}
cat("- Alpha多样性箱线图: alpha_diversity_plot.png\n")
