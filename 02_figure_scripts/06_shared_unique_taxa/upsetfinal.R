...
# 设置黑白灰的颜色方案
gray_colors <- c("#E0E0E0", "#B0B0B0", "#808080", "#505050", "#202020")
black_colors <- c("#000000", "#333333", "#666666", "#999999")

# 创建简单的黑白查询（只高亮主要的交集）
# 这里我们只高亮全部四组的交集，使用深灰色
simple_queries <- list(
  list(
    query = intersects, 
...
common_par <- list(
  ...
  sets.bar.color  = group_colors[colnames(group_binary)],  # 使用指定的分组颜色
  mb.ratio        = c(0.6, 0.4),
  # 优化刻度显示：c(intersection size title, intersection size tick labels, set size title, set size tick labels, set names, numbers above bars)
  text.scale      = c(1.5, 1.3, 1.5, 1.3, 1.5, 1.0),  # 调整刻度
  queries = simple_queries,  # 使用简化的查询
  # 矩阵点颜色设置（黑白灰）
  matrix.color = "black",
  shade.color = "#E0E0E0",  # 浅灰色
  shade.alpha = 0.25
)

# 6.1 位图（保存到upset文件夹）- 增加分辨率
png(file.path(output_dir, "upset_simple.png"), width = 3200, height = 2200, res = 350)
do.call(what = upset, args = common_par)
dev.off()

# 6.2 矢量图（保存到upset文件夹）- 增加尺寸
pdf(file.path(output_dir, "upset_simple.pdf"), width = 12, height = 8)
do.call(what = upset, args = common_par)
dev.off()
