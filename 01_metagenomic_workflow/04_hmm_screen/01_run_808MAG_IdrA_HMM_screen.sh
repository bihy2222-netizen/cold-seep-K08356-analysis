...
if [[ "$#" -lt 3 ]]; then
  printf 'Usage: %s FAA_DIR MODEL_DIR WORK_DIR [CPU] [EXPECTED_MAG_COUNT] [EXPECTED_PROTEIN_COUNT]\n' "$0" >&2
  exit 2
fi
...
WORK_DIR="$3"
CPU="${4:-${CPU:-32}}"
EXPECTED_MAG_COUNT="${5:-808}"
EXPECTED_PROTEIN_COUNT="${6:-0}"
HMMSEARCH="${HMMSEARCH:-$(command -v hmmsearch || true)}"

if [[ -z "$HMMSEARCH" || ! -x "$HMMSEARCH" ]]; then
  printf 'hmmsearch was not found; activate an HMMER environment or set HMMSEARCH.\n' >&2
  exit 1
fi
...
if [[ "${#faa_files[@]}" -ne "$EXPECTED_MAG_COUNT" ]]; then
  printf 'Expected %s FAA files, found %s\n' "$EXPECTED_MAG_COUNT" "${#faa_files[@]}" >&2
  exit 1
fi
...
if command -v sha256sum >/dev/null 2>&1; then
  ...
  shasum -a 256 "$MODEL_COPY_DIR"/*.hmm > "$TABLE_DIR/model_sha256.tsv"
fi
...
if [[ "$EXPECTED_PROTEIN_COUNT" -gt 0 && "$protein_count" -ne "$EXPECTED_PROTEIN_COUNT" ]]; then
  printf 'Expected %s proteins, found %s\n' "$EXPECTED_PROTEIN_COUNT" "$protein_count" >&2
  exit 1
fi
...
{
  printf 'screen\thits\tunique_MAGs\tunique_proteins\n'
  awk -F '\t' 'BEGIN{OFS="\t"} NR>1 {hits[$1]++; mags[$1 SUBSEP $2]=1; proteins[$1 SUBSEP $4]=1} END {for (s in hits) {m=0;p=0; for (k in mags) {split(k,a,SUBSEP); if(a[1]==s)m++} for(k in proteins){split(k,a,SUBSEP);if(a[1]==s)p++} print s,hits[s],m,p}}' \
    "$TABLE_DIR/all_T640_hits_long.tsv" | sort
} > "$TABLE_DIR/T640_screen_summary.tsv"
