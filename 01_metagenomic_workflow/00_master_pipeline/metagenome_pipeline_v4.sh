...
#
# Version status:
#   Metagenome main workflow v1.2 + K08356 classification framework v1.1.
#   v4 adds reference integration for AioA/IdrA/DMSOR evidence.
#
...
EOF
  ...
  cat > "${PROJECT_DIR}/metadata/reference_protein_manifest.v4_template.tsv" <<'EOF'
sequence_id	family	role	accession	evidence_level	source	fasta_file	include_in_aioa_seed	include_in_idra_seed	include_in_calibration	include_in_tree	note
example_AioA_exp	AioA	seed	NA	experimentally_validated	literature	/path/to/aioA_refs.faa	yes	no	no	yes	Replace with curated experimentally supported AioA
example_IdrA_IR12	IdrA	seed	NA	experimentally_validated	literature	/path/to/idrA_refs.faa	no	yes	no	yes	Include Denitromonas sp. IR-12 IdrA
example_IdrA_SCT	IdrA	calibration_positive	NA	experimentally_validated	literature	/path/to/idrA_refs.faa	no	no	yes	yes	Use independent IdrA validation if not used for seed
example_ArxA	ArxA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR near-neighbor negative reference
example_ArrA	ArrA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR near-neighbor negative reference
example_DmsA	DmsA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_DorA	DorA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_NarG	NarG	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_NapA	NapA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_PsrA	PsrA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_PhsA	PhsA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_TtrA	TtrA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
example_SerA	SerA	negative_reference	NA	curated	literature	/path/to/dmsor_outgroups.faa	no	no	yes	yes	DMSOR outgroup
EOF
  if [[ ! -f "${REFERENCE_MANIFEST}" ]]; then
    cp "${PROJECT_DIR}/metadata/reference_protein_manifest.v4_template.tsv" "${REFERENCE_MANIFEST}"
  fi

  cat > "${PROJECT_DIR}/references/templates/reference_family_minimums.tsv" <<'EOF'
family	min_for_tree	min_for_calibration	note
AioA	5	3	Use diverse experimentally supported or strongly curated AioA
IdrA	3	2	Include IR-12, SCT and ISMEJ high-confidence IdrA where possible
ArxA	2	1	Near-neighbor DMSOR family
ArrA	2	1	Near-neighbor DMSOR family
DmsA	2	1	Outgroup DMSOR family
DorA	1	1	Outgroup DMSOR family
NarG	2	1	Outgroup DMSOR family
NapA	2	1	Outgroup DMSOR family
PsrA	1	1	Outgroup DMSOR family
PhsA	1	1	Outgroup DMSOR family
TtrA	1	1	Outgroup DMSOR family
SerA	1	1	Outgroup DMSOR family
EOF
...
check_k08356_references() {
  ...
  for f in "${PFAM00384_HMM}" "${RIESKE_PF00355_HMM}" "${DIHAEM_PF03150_HMM}" \
           ...
           "${REFERENCE_MANIFEST}"; do
    if [[ ! -s "${f}" ]]; then echo "Missing/empty K08356 reference: ${f}" >&2; failed=1; fi
  done
  [[ "${failed}" == 0 ]] || return 1
  awk 'BEGIN{FS="\t"; ok=1}
    NR==1 {
      required="sequence_id family role accession evidence_level source fasta_file include_in_aioa_seed include_in_idra_seed include_in_calibration include_in_tree note"
      n=split(required, req, " ")
      for(i=1;i<=NF;i++) seen[$i]=1
      for(i=1;i<=n;i++) if(!(req[i] in seen)){print "Missing manifest column: " req[i] > "/dev/stderr"; ok=0}
      exit ok?0:1
    }' "${REFERENCE_MANIFEST}"
...
integrate_k08356_references() {
  ...
  #
  # The FASTA headers in fasta_file must start with sequence_id from the manifest.
  # Training/seed and calibration should be separated where possible to avoid
  # circular threshold validation.
  activate_env seqkit
  run "mkdir -p '${INTEGRATED_REFERENCE_DIR}'"
  run "awk 'BEGIN{FS=OFS=\"\\t\"; ok=1} NR==1 {required=\"sequence_id family role accession evidence_level source fasta_file include_in_aioa_seed include_in_idra_seed include_in_calibration include_in_tree note\"; n=split(required, req, \" \"); for(i=1;i<=NF;i++) seen[\$i]=1; for(i=1;i<=n;i++) if(!(req[i] in seen)){print \"Missing manifest column: \" req[i] > \"/dev/stderr\"; ok=0}; exit ok?0:1}' '${REFERENCE_MANIFEST}'"
  run "awk 'BEGIN{FS=OFS=\"\\t\"} NR==1 {for(i=1;i<=NF;i++) h[\$i]=i; next} NR>1 && \$1 !~ /^example_/ && \$(h[\"fasta_file\"]) !~ /^\\/path\\/to\\// {print \$(h[\"fasta_file\"])}' '${REFERENCE_MANIFEST}' | sort -u > '${INTEGRATED_REFERENCE_DIR}/source_fasta_files.txt'"
  run "while read -r fasta; do [[ -s \"\$fasta\" ]] || { echo \"Missing reference FASTA: \$fasta\" >&2; exit 1; }; cat \"\$fasta\"; done < '${INTEGRATED_REFERENCE_DIR}/source_fasta_files.txt' > '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa'"
  run "awk '/^>/{id=\$1; sub(/^>/,\"\",id); if(seen[id]++){print \"Duplicate reference FASTA ID: \" id > \"/dev/stderr\"; dup=1}} END{exit dup?1:0}' '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa'"

  run "awk 'BEGIN{FS=OFS=\"\\t\"} NR==1 {for(i=1;i<=NF;i++) h[\$i]=i; next} NR>1 && \$1 !~ /^example_/ && \$(h[\"fasta_file\"]) !~ /^\\/path\\/to\\// {id=\$(h[\"sequence_id\"]); if(\$(h[\"include_in_aioa_seed\"])==\"yes\") print id > \"'${INTEGRATED_REFERENCE_DIR}'/aioa_seed.ids\"; if(\$(h[\"include_in_idra_seed\"])==\"yes\") print id > \"'${INTEGRATED_REFERENCE_DIR}'/idra_seed.ids\"; if(\$(h[\"include_in_calibration\"])==\"yes\") print id > \"'${INTEGRATED_REFERENCE_DIR}'/calibration.ids\"; if(\$(h[\"include_in_tree\"])==\"yes\") print id > \"'${INTEGRATED_REFERENCE_DIR}'/tree.ids\"}' '${REFERENCE_MANIFEST}'"

  run "seqkit grep -n -f '${INTEGRATED_REFERENCE_DIR}/aioa_seed.ids' '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa' > '${AIOA_SEED_FAA}'"
  run "seqkit grep -n -f '${INTEGRATED_REFERENCE_DIR}/idra_seed.ids' '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa' > '${IDRA_SEED_FAA}'"
  run "seqkit grep -n -f '${INTEGRATED_REFERENCE_DIR}/tree.ids' '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa' > '${DMSOR_REFERENCE_FAA}'"
  run "seqkit grep -n -f '${INTEGRATED_REFERENCE_DIR}/calibration.ids' '${INTEGRATED_REFERENCE_DIR}/all_reference_proteins.raw.faa' > '${INTEGRATED_REFERENCE_DIR}/calibration_references.faa'"

  run "awk 'BEGIN{FS=OFS=\"\\t\"; print \"family\", \"role\", \"n\", \"include_in_aioa_seed\", \"include_in_idra_seed\", \"include_in_calibration\", \"include_in_tree\"} NR==1 {for(i=1;i<=NF;i++) h[\$i]=i; next} NR>1 && \$1 !~ /^example_/ && \$(h[\"fasta_file\"]) !~ /^\\/path\\/to\\// {key=\$(h[\"family\"]) OFS \$(h[\"role\"]); n[key]++; a[key]+=(\$(h[\"include_in_aioa_seed\"])==\"yes\"); b[key]+=(\$(h[\"include_in_idra_seed\"])==\"yes\"); c[key]+=(\$(h[\"include_in_calibration\"])==\"yes\"); d[key]+=(\$(h[\"include_in_tree\"])==\"yes\")} END{for(key in n) print key, n[key], a[key]+0, b[key]+0, c[key]+0, d[key]+0}' '${REFERENCE_MANIFEST}' > '${INTEGRATED_REFERENCE_DIR}/reference_family_role_counts.tsv'"
  run "md5sum '${AIOA_SEED_FAA}' '${IDRA_SEED_FAA}' '${DMSOR_REFERENCE_FAA}' '${INTEGRATED_REFERENCE_DIR}/calibration_references.faa' > '${INTEGRATED_REFERENCE_DIR}/integrated_reference_md5.tsv'"
  run "cp '${REFERENCE_MANIFEST}' '${INTEGRATED_REFERENCE_DIR}/reference_protein_manifest.used.tsv'"
  run "seqkit stats '${AIOA_SEED_FAA}' '${IDRA_SEED_FAA}' '${DMSOR_REFERENCE_FAA}' '${INTEGRATED_REFERENCE_DIR}/calibration_references.faa' > '${INTEGRATED_REFERENCE_DIR}/integrated_reference_seqkit_stats.tsv'"
}
...
aioa_idra_dual_hmmsearch() {
  ...
  local calibration_refs="${INTEGRATED_REFERENCE_DIR}/calibration_references.faa"
  [[ -s "${calibration_refs}" ]] || calibration_refs="${DMSOR_REFERENCE_FAA}"
  ...
  run "hmmsearch --cpu '${THREADS}' --domtblout '${PROJECT_DIR}/08_annotation/hmm_calibration/idrA.references.domtblout' '${PROJECT_DIR}/08_annotation/hmm/idrA.hmm' '${calibration_refs}' > '${LOG_DIR}/idrA.references.hmmsearch.log' 2>&1"
  run "hmmsearch --cpu '${THREADS}' --domtblout '${PROJECT_DIR}/08_annotation/hmm_calibration/aioA.references.domtblout' '${PROJECT_DIR}/08_annotation/hmm/aioA.hmm' '${calibration_refs}' > '${LOG_DIR}/aioA.references.hmmsearch.log' 2>&1"
}
...
Steps:
  ...
  reference-integrate   Integrate curated AioA/IdrA/DMSOR reference sets
...
main() {
  ...
  case "${step}" in
    ...
    reference-integrate) integrate_k08356_references ;;
