...
#
# Version status:
#   Metagenome main workflow v1.1 + K08356 classification framework v0.8.
#
...
ensure_project_dirs() {
  mkdir -p "${PROJECT_DIR}"/{metadata,logs,00_raw/sra,00_raw/fastq,01_qc/fastqc_raw,01_qc/fastqc_clean,01_qc/fastp,02_clean,03_assembly,03_assembly_qc,04_metaphlan,05_binning,06_genes/{CDS,AA,GFF,catalog},07_abundance/{gene,MAG},08_annotation/{kofam/tmp,k08356,hmm,hmm_calibration,neighborhood},09_MAGs/{all_refined_bins,filtered_MAGs,dRep_out,checkm2,gunc,gtdbtk},10_statistics,reports,references}
}
...
predict_genes() {
  ...
  sample_col sample | while read -r sample; do
    run "prodigal -i '${PROJECT_DIR}/03_assembly/${sample}/final.contigs.fa' -d '${PROJECT_DIR}/06_genes/CDS/${sample}_cds.fa' -a '${PROJECT_DIR}/06_genes/AA/${sample}_AA.faa' -f gff -o '${PROJECT_DIR}/06_genes/GFF/${sample}.gff' -p meta -m > '${LOG_DIR}/${sample}.prodigal.log' 2>&1"
  done
}
...
build_gene_catalog() {
  ...
  run "awk 'BEGIN{OFS=\"\\t\"; print \"catalog_gene_id\", \"representative_CDS_id\", \"representative_protein_id\", \"member_gene_id\", \"sample\", \"contig\", \"start\", \"end\", \"strand\"} /^>Cluster/ {cluster=\$2; rep=\"\"; next} /^[0-9]/ {line=\$0; id=line; sub(/^.*>/, \"\", id); sub(/\\.\\.\\..*$/, \"\", id); if (line ~ /\\*/) rep=id; members[cluster]=members[cluster] id \"\\n\"; reps[cluster]=rep} END {for (c in members) {split(members[c], a, \"\\n\"); for (i in a) if (a[i] != \"\") {sample=a[i]; sub(/_.*/, \"\", sample); print \"catalog_\" c, reps[c], reps[c], a[i], sample, \"NA\", \"NA\", \"NA\", \"NA\"}}}' '${PROJECT_DIR}/06_genes/catalog/all_samples_cds.nr95.fa.clstr' > '${PROJECT_DIR}/06_genes/catalog/catalog_cds_protein_member_map.tsv'"

  # Add genomic coordinates from GFF when IDs match Prodigal IDs.
  run "awk 'BEGIN{FS=OFS=\"\\t\"} FNR==NR && \$0 !~ /^#/ {id=\"\"; split(\$9, a, \";\"); for(i in a){if(a[i] ~ /^ID=/){id=a[i]; sub(/^ID=/,\"\",id)}} if(id != \"\") coord[id]=\$1 OFS \$4 OFS \$5 OFS \$7; next} NR==1 {print; next} {if(\$4 in coord){split(coord[\$4], c, OFS); \$6=c[1]; \$7=c[2]; \$8=c[3]; \$9=c[4]} print}' '${PROJECT_DIR}'/06_genes/GFF/*.gff '${PROJECT_DIR}/06_genes/catalog/catalog_cds_protein_member_map.tsv' > '${PROJECT_DIR}/06_genes/catalog/catalog_cds_protein_member_map.with_coords.tsv'"
}
...
k08356_candidate_search() {
  ...
  run "awk 'BEGIN{FS=OFS=\"\\t\"} /K08356/ {print}' '${PROJECT_DIR}/08_annotation/kofam/all_samples_AA.from_cds_nr95.kofamscan.tsv' > '${PROJECT_DIR}/08_annotation/k08356/K08356.kofam_hits.tsv' || true"
  ...
  # KOfam detail-tsv formats differ across releases; this keeps IDs from any
  # K08356 line and records the full original line for manual audit.
  run "awk 'BEGIN{FS=OFS=\"\\t\"; print \"candidate_id\", \"source\", \"kofam_raw_line\"} NR>1 && /K08356/ {id=\$1; gsub(/^\\*/, \"\", id); print id, \"KOfam_K08356\", \$0}' '${PROJECT_DIR}/08_annotation/k08356/K08356.kofam_hits.tsv' > '${PROJECT_DIR}/08_annotation/k08356/K08356.filtered_hits.tsv' || true"
...
k08356_calibrate_hmm_thresholds() {
  ...
  run "printf 'threshold_version\\tmin_domain_score\\tmin_domain_cov\\tmin_score_delta\\tgray_zone_delta\\tcalibration_note\\n' > '${out}'"
  run "printf 'manual_v0.1\\t50\\t0.30\\t10\\t10\\tPLACEHOLDER: replace after positive-negative reference calibration and leave-one-out validation\\n' >> '${out}'"

  run "awk 'BEGIN{FS=OFS=\"\\t\"; print \"family\", \"sequence_id\", \"full_evalue\", \"full_score\", \"domain_ievalue\", \"domain_score\", \"target_len\", \"hmm_from\", \"hmm_to\", \"ali_from\", \"ali_to\", \"domain_cov\"} \$0 !~ /^#/ {cov=(\$19-\$18+1)/\$3; print \"AioA\", \$1, \$7, \$8, \$13, \$14, \$3, \$16, \$17, \$18, \$19, cov}' '${PROJECT_DIR}/08_annotation/k08356/aioA.domtblout' > '${PROJECT_DIR}/08_annotation/hmm_calibration/aioA_candidate_scores.tsv'"
  run "awk 'BEGIN{FS=OFS=\"\\t\"; print \"family\", \"sequence_id\", \"full_evalue\", \"full_score\", \"domain_ievalue\", \"domain_score\", \"target_len\", \"hmm_from\", \"hmm_to\", \"ali_from\", \"ali_to\", \"domain_cov\"} \$0 !~ /^#/ {cov=(\$19-\$18+1)/\$3; print \"IdrA\", \$1, \$7, \$8, \$13, \$14, \$3, \$16, \$17, \$18, \$19, cov}' '${PROJECT_DIR}/08_annotation/k08356/idrA.domtblout' > '${PROJECT_DIR}/08_annotation/hmm_calibration/idrA_candidate_scores.tsv'"
}
...
k08356_classify_by_hmm() {
  ...
  run "awk 'BEGIN{FS=OFS=\"\\t\"} FNR==NR && NR>1 {min_score=\$2; min_cov=\$3; min_delta=\$4; gray=\$5; version=\$1; next} ARGIND==2 && FNR>1 {aio_score[\$2]=\$6; aio_full[\$2]=\$4; aio_cov[\$2]=\$12; ids[\$2]=1; next} ARGIND==3 && FNR>1 {idr_score[\$2]=\$6; idr_full[\$2]=\$4; idr_cov[\$2]=\$12; ids[\$2]=1; next} END {print \"sequence_id\", \"AioA_full_score\", \"AioA_domain_score\", \"AioA_cov\", \"IdrA_full_score\", \"IdrA_domain_score\", \"IdrA_cov\", \"score_delta\", \"HMM_class\", \"threshold_version\"; for(id in ids){as=(id in aio_score)?aio_score[id]:0; is=(id in idr_score)?idr_score[id]:0; ac=(id in aio_cov)?aio_cov[id]:0; ic=(id in idr_cov)?idr_cov[id]:0; delta=as-is; cls=\"unknown_by_HMM\"; if(as >= min_score && ac >= min_cov && delta >= min_delta) cls=\"AioA_preliminary\"; else if(is >= min_score && ic >= min_cov && -delta >= min_delta) cls=\"IdrA_preliminary\"; else if((as >= min_score || is >= min_score) && (delta < gray && delta > -gray)) cls=\"gray_zone_unknown\"; print id, aio_full[id]+0, as+0, ac+0, idr_full[id]+0, is+0, ic+0, delta, cls, version}}' '${threshold_file}' '${PROJECT_DIR}/08_annotation/hmm_calibration/aioA_candidate_scores.tsv' '${PROJECT_DIR}/08_annotation/hmm_calibration/idrA_candidate_scores.tsv' > '${out}'"
}
