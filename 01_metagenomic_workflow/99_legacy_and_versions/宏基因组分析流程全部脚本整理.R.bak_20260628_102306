...
#检测去冗余后MAG的质量和污染度
#(checkm2) 
ps@ps:~/ps1/data/bihongyu/cold_seep/illu/binning/all_50_10_dRep$ nohup checkm2 predict -x fasta -i dereplicated_genomes -o checkm2_result -t 64 &
########后边没有使用checkm2的结果
...
nohup mafft --quiet --auto gtdbtk.bac120.user_msa.fasta > gtdbtk.bac120.user_msa.mafft.fa &
  nohup mafft --quiet --auto gtdbtk.ar53.user_msa.fasta > gtdbtk.ar53.user_msa.mafft.fa &
#####################################  
  conda activate trimal
trimal -in gtdbtk.ar53.user_msa.mafft.fa -out gtdbtk.ar53.user_msa.mafft.fa -automated1
trimal -in gtdbtk.bac120.user_msa.mafft.fasta -out gtdbtk.bac120.user_msa.mafft.trimal.fasta -automated1
######################################################建树用MAG结果构建的蛋白树mag ########
##########使用的是gtdbtk产生的细菌和古菌的通用蛋白然后对齐 剪切 iqtree建树还要对齐剪切，处理数据是不是很麻烦，手动对齐么～
#######需要自己有写命令/可以问问ai
#######3跑出来的 bins 的结果是蛋白质序列，然后 iqtree里面的算法自动识别蛋白质序列关键基因注释后构建的蛋白质树么###
#####也可以用gtdb infer好像是也是建树，会快一点，脚本需要后面自己AI写一个脚本
...
#############从bin_refine结果中提取drep后bin的详细信息
(base) ps@ps:~/ps1/data/bihongyu/cold_seep/illu/binning$ # 创建总表头
...
# 2. 去掉 "_bins" 后缀，得到真正的样本名 (SY459WG-4-8)
sample_name=${raw_folder%_bins}

# 3. 处理文件内容：将 bin. 替换为 样本名_bin
# 注意：NR>1 是为了跳过每个文件自带的表头
